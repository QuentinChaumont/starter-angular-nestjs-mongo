 ...
 * dialog, the `/legal/cookies` + `/legal/privacy` + `/legal/notice` routes
 * with legal-page templates, and the `<lib-legal-links>` footer. Mounts
 * `<lib-consent-banner>` and `<lib-legal-links>` in `app.ts` (outside the
 * router outlet). "Manage cookies" appears in the dashboard menu
 * automatically via the `CONSENT_MANAGER` hook. Requires `frontend-design`.
 * Idempotent.
 */
export default async function frontendConsentGenerator(
  ...
  // The always-visible legal footer (legal notice / privacy / cookies).
  ensureNamedImport(
    tree,
    APP_COMPONENT_PATH,
    'LegalLinks',
  ...
  ensureArrayItem(tree, APP_COMPONENT_PATH, 'imports', 'LegalLinks');
  ...
  if (template !== null && !template.includes('lib-legal-links')) {
